...
app.config(function(sgtDataProvider){
    sgtDataProvider.api_key = "rcXQrzcxRC";
...
app.controller('ioController', function(sgtData, $log){
    ...
    self_controller.data = {};
    //Add a function called getData to your controller to call the SGT API
    //Refer to the prototype instructions for more help
    self_controller.getData = function(){
        sgtData.call_api()
            .then(
                function(response){
                    $log.log("api call from controller success", response);
                    self_controller.data = response.data;
                },
                function(response){
                    $log.log("api call from controller failed", response)
                }
            )
